  ...
  try {
    ...
    if (autoTranslate !== undefined) {
        ...
        try {
          ...
          if (repository.user.accessToken) {
            ...
            return NextResponse.json(
              {
                error:
                  "GitHub authentication failed. Please reinstall the GitHub App.",
              },
              { status: 401 },
            );
          }
      ...
      if (autoTranslate && !currentAutoTranslate) {
        console.log(
          `[Config] Enabling auto-translate for ${repository.fullName}`,
        );
        ...
        try {
          ...
          if (error.status === 422) {
            return NextResponse.json(
              {
                error:
                  "Webhook already exists or configuration error. Please check your repository settings.",
              },
              { status: 422 },
            );
          }
      ...
      if (!autoTranslate && currentAutoTranslate && webhookId) {
        console.log(
          `[Config] Disabling auto-translate for ${repository.fullName}`,
        );
