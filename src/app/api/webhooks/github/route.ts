...
export async function POST(request: NextRequest) {
  try {
    ...
    if (branchName !== repository.default_branch) {
      console.log(
        `[Webhook] Ignoring push to non-default branch: ${branchName}`,
      );
    ...
    if (!repo.config) {
      console.log(
        `[Webhook] Repository not configured: ${repository.full_name}`,
      );
    ...
    if (!repo.config.autoTranslate) {
      console.log(
        `[Webhook] Auto-translate disabled for: ${repository.full_name}`,
      );
    ...
    console.log(
      `[Webhook] Changed files: ${Array.from(changedFiles).join(", ")}`,
    );
...
function filterFilesToTranslate(
  ...
  return files.filter((file) => {
    ...
    if (
      excludePaths &&
      excludePaths.some((pattern) => matchPath(file, pattern))
    ) {
      return false;
    }
