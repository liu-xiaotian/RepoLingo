...
export default function RepoConfigPage({
  ...
  const handleDirSelect = (dirPath: string) => {
    ...
    const allSelected = filesInDir.every((file) =>
      selectedFiles.includes(file),
    );
    if (allSelected) {
      // 取消选中所有
      setSelectedFiles(
        selectedFiles.filter((file) => !filesInDir.includes(file)),
      );
    } else {
      // 选中所有（合并去重）
  ...
  const getDirSelectState = (dirPath: string): "all" | "some" | "none" => {
    ...
    const selectedCount = filesInDir.filter((file) =>
      selectedFiles.includes(file),
    ).length;
  ...
  const getAllDirPaths = (nodes: FileNode[]): string[] => {
    return nodes.flatMap((node) => {
      if (node.type === "dir") {
        return [
          node.path,
          ...(node.children ? getAllDirPaths(node.children) : []),
        ];
      }
      return [];
    });
          ...
          <div>
            <div className="flex items-center gap-1 py-1.5 px-2 font-medium text-sm hover:bg-accent rounded">
              ...
              <input
                ...
                ref={(el) => {
                  if (el)
                    el.indeterminate = getDirSelectState(node.path) === "some";
                }}
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                    {SUPPORTED_AI_MODELS.filter((m) => m.recommended).map(
                      (model) => (
                        <button
                          key={model.id}
                  ...
                  <div className="p-4 bg-primary/5 border border-primary/20 rounded-lg space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <div className="space-y-1">
                        ...
                        <p className="text-xs text-muted-foreground">
                          系统会自动为此仓库创建
                          Webhook，监听代码推送事件。当检测到翻译范围内的文档变更时，将自动触发增量翻译任务。
                        </p>
                      </div>
                    </div>
          ...
          <Card className="mt-6 md:mt-8">
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                <div>
                  ...
                  <div className="font-medium">
                    {SUPPORTED_LANGUAGES.find((l) => l.code === baseLanguage)
                      ?.name || baseLanguage}
                  </div>
                </div>
              ...
              <div>
                ...
                <div className="font-medium">
                  {aiModel ? (
                    SUPPORTED_AI_MODELS.find((m) => m.id === aiModel)?.name ||
                    aiModel
                  ) : (
